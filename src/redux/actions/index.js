...
const actions = {
  ...
  getGreys: (keyword, completed, failed) => {
    return (dispatch) => {
      ...
      Api.get(
        ...
        (success) => {
          console.log('greyApiSUCCESSSS', success)
          dispatch({
   ...
   getGreyFabricInProcess: (keyword, completed, failed) => {
    return (dispatch) => {
      dispatch({ type: actionTypes.START_ACTIVITY_LOADING });
      Api.get(
        `fabricInProcess.php/?ubi=${keyword}`,
        (success) => {
  ...
  //Getting Grey Fabric Out Standing
  getGreyFabOutStanding: (keyword, completed, failed) => {
    console.log('TESTSTSTESETSETST')
    return (dispatch) => {
      dispatch({ type: actionTypes.START_ACTIVITY_LOADING });
      Api.get(
        `greyFabricOutStanding.php/?contract=${keyword}`,

        (success) => {
          console.log('getfabricoutstandingSUCCESSSS', success)
          dispatch({
            type: actionTypes.GREY_FABRIC_OUSTANDING,
            payload: success.greyFabOutStanding,
          });

          dispatch({ type: actionTypes.CLOSE_ACTIVITY_LOADING });
        },
        (error) => {
          console.log('getfabricoutstanding error', error)
          dispatch({ type: actionTypes.CLOSE_ACTIVITY_LOADING });
        },
      );
    };
  },
  //Getting Grey Fabric Stock In Hand
  getGreyFabricStockInHand:(keyword) => {
    console.log('greyfabricstockinhand')
    return (dispatch) => {
      dispatch({type:actionTypes.START_ACTIVITY_LOADING})
      Api.get(
        `greyFabricStockInHand.php/?description=${keyword}`,
        (success) => {
          console.log('greyFabStockInHandSuccessfully',success)
          dispatch({
            type:actionTypes.GREY_FABRIC_STOCK_IN_HAND,
            payload:success.greyFabStock,
          })
          dispatch({type:actionTypes.CLOSE_ACTIVITY_LOADING})
        },
        (error) => {
          console.log('greyFabStockInHandError',error)
          dispatch({type:actionTypes.CLOSE_ACTIVITY_LOADING})
        }
      )
    }
